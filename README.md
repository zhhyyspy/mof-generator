...
**V3.3** 面向业务用户的**可视化模型编辑**:
- **Phase 1**: 类卡片 hover 快捷按钮 + 图标化属性构建器 (6 种语义类型) + 向导式关联构建器 + 依赖感知的安全删除 + 一键属性模板
- **Phase 2**: 5 步新建类向导 + 属性模板库扩展至 7 组 (基础/设备/工程/合同/人员/审批流/组织)
- **显示优化**: 全链路卡片/列表 "中文 label 为主, 英文 name 为次" — 业务用户一眼看到的是"抽蓄电站"而非"PumpedStoragePlant"

**V3.4** **Excel/CSV 结构化表格 → M1 抽取** (AI 辅助):
- 上传 .xlsx/.xls/.csv → 文档卡自动出现 📊 从表格抽取 M1 按钮
- **AI 解读表格结构**:自动识别多级表头、数据起止、汇总行、装饰列、合并单元格
- **工作簿级分类**: AI 判断哪些 sheet 是数据表、哪些是目录/说明,自动跳过非数据 sheet (并行 LLM 调用)
- **交互式预览**: 原样渲染 Excel 表格并高亮标记表头 (🔵)、数据区 (🟢)、汇总行 (🟠)、标题行 (⚫);点击行号可重新标记,点击列号可切换"忽略/使用"
- **M0 示例数据可选保存**: 勾选"保存前 N 行作为示例数据",真实行被存入 `MOFClass.sample_instances`,便于后续审查/校准

抛弃"一堆技术参数表单",每个操作都是"所见即所得"。
...
**显示优化** (业务视角): 所有卡片/列表的"中文 label"与"英文 name"位置全面对调:
- **主树卡片**: 14px 加粗 `抽蓄电站` · 小号 monospace 胶囊 `PumpedStoragePlant`
- **属性行**: 加粗中文属性名 · 小号灰色英文 name
- **枚举/关联/同义/M2 关系管理/审查面板**: 全部统一此规则
- 无中文 label 的 legacy 数据自动 fallback 到只显 name,不出现空行
